...
RSpec.describe Note, type: :model do
  ...
  describe '.all_active_resource' do
    before :each do
      ...
      @note3 = Note.create(id: 3, user_id: 1, table_key: 2, table_name: "Resources", content: "Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur", created_at: "2019-07-05 20:35:50", updated_at: "2019-07-10 20:35:50")

    end
    it 'returns active resources' do
      expect(Note.notes_by_resource(1)).to eq([@note1, @note2])
      expect(Note.notes_by_resource(1)).to_not eq([@note3])
    end
  end
end
